...
export default function FindUsers() {
  ...
  return (
    <section className="bg-gray-100" data-testid="find-users">
      <div className="bg-white w-full md:max-w-lg min-h-lvh mx-auto p-6">
        <div className="flex gap-4 mb-4 flex-col">
          <input 
            ...
            className="p-4 border-3 border-gray-200 rounded bg-gray-100"
            aria-label="Search GitHub users"
          />
          <button
            onClick={handleSearch}
            className="py-4 bg-sky-500 text-white rounded cursor-pointer"
            disabled={isLoading}
            aria-label="Search"
          >
            ...
            {data?.pages?.map((page, pageIndex) => {
              return (
                <div key={pageIndex} className="flex flex-col gap-4">
                  {page?.items?.map((user, index) => <CardDetails key={index} login={user?.login} />)}
                </div>
              );
            })}
          </div>
          {isFetchingNextPage && <p className="text-center py-4 text-sky-600">Loading more...</p>}
